...
class FormPhp
{
    ...
    public function generate($postDataSummary, $postData) {
        ...
        if ( ! empty($postData['basic'])) {
            ...
            foreach ($postData['basic'] as $k => $v) {
                ...
                if ($v['fieldType'] == 'link') {
                    ...
                    $code .= BlockBuilderUtility::tab(4).'<div class="col-xs-12">'.PHP_EOL;
                    $code .= BlockBuilderUtility::tab(5).'<?php echo Core::make(\'helper/concrete/asset_library\')->file(\''.$v['handle'].'_link_from_file_manager-\'.$uniqueID, $view->field(\''.$v['handle'].'_link_from_file_manager\'), t(\'Choose File\'), !empty($'.$v['handle'].'[\'link_from_file_manager\']) ? File::getByID($'.$v['handle'].'[\'link_from_file_manager\']) : null); ?>'.PHP_EOL;
                    $code .= BlockBuilderUtility::tab(4).'</div>'.PHP_EOL;
                    $code .= BlockBuilderUtility::tab(3).'</div>'.PHP_EOL.PHP_EOL;
                    ...
                    $code .= BlockBuilderUtility::tab(5).'<?php echo $form->select($view->field(\''.$v['handle'].'_new_window\'), array(\'0\'=>t(\''.addslashes($postData['noLabel']).'\'), \'1\'=>t(\''.addslashes($postData['yesLabel']).'\')), $'.$v['handle'].'[\'new_window\']); ?>'.PHP_EOL;
                    $code .= BlockBuilderUtility::tab(4).'</div>'.PHP_EOL;
                    $code .= BlockBuilderUtility::tab(3).'</div>'.PHP_EOL.PHP_EOL;
                ...
                if ($v['fieldType'] == 'link_from_sitemap') {
                    ...
                    if ( ! empty($v['linkFromSitemapShowNewWindowField'])) {
                        ...
                        $code .= BlockBuilderUtility::tab(3) . '<?php echo $form->select($view->field(\'' . $v['handle'] . '_new_window\'), array(\'0\'=>t(\''.addslashes($postData['noLabel']).'\'), \'1\'=>t(\''.addslashes($postData['yesLabel']).'\')), $' . $v['handle'] . '_new_window); ?>' . PHP_EOL;
                        $code .= BlockBuilderUtility::tab(2) . '</div>' . PHP_EOL . PHP_EOL;
                    }
                ...
                if ($v['fieldType'] == 'link_from_file_manager') {
                    ...
                    $code .= BlockBuilderUtility::tab(3) . '<?php echo Core::make(\'helper/concrete/asset_library\')->file(\'' . $v['handle'] . '-\'.$uniqueID, $view->field(\'' . $v['handle'] . '\'), t(\'Choose File\'), !empty($' . $v['handle'] . ') ? File::getByID($' . $v['handle'] . ') : null); ?>' . PHP_EOL;
                    $code .= BlockBuilderUtility::tab(2) . '</div>' . PHP_EOL . PHP_EOL;
        ...
        if ( ! empty($postData['entries'])) {
            ...
            $code .= BlockBuilderUtility::tab(5) . '<button type="button" class="entry-header-action entry-header-duplicate-entry js-duplicate-entry" title="<?php echo t(\''.addslashes($postData['duplicateEntryLabel']).'\'); ?>"><i class="fa fa-copy"></i></button>' . PHP_EOL;
            ...
            foreach ($postData['entries'] as $k => $v) {
                ...
                if ($v['fieldType'] == 'wysiwyg_editor') {
                    ...
                    if ($height) {
                        ...
                        $code .= BlockBuilderUtility::tab(7) . '.js-custom-editor-height-<?php echo str_replace(array(\'[\', \']\'), array(\'\[\', \'\]\'), $view->field(\'entry\').\'[<%=_.escape(position)%>][' . $v['handle'] . ']\'); ?>-<?php echo $uniqueID; ?> .cke_contents {' . PHP_EOL;
                        $code .= BlockBuilderUtility::tab(8) . 'height: '.$height.'px !important;' . PHP_EOL;
                        $code .= BlockBuilderUtility::tab(7) . '}' . PHP_EOL;
                        $code .= BlockBuilderUtility::tab(7) . '.js-custom-editor-height-<?php echo str_replace(array(\'[\', \']\'), array(\'\[\', \'\]\'), $view->field(\'entry\').\'[<%=_.escape(position)%>][' . $v['handle'] . ']\'); ?>-<?php echo $uniqueID; ?> .cke_editable {' . PHP_EOL;
                        $code .= BlockBuilderUtility::tab(8) . 'min-height: '.$height.'px !important;' . PHP_EOL;
                        $code .= BlockBuilderUtility::tab(7) . '}' . PHP_EOL;
                        $code .= BlockBuilderUtility::tab(7) . '<?php else :?>' . PHP_EOL;
                        $code .= BlockBuilderUtility::tab(7) . '.js-custom-editor-height-<?php echo str_replace(array(\'[\', \']\'), array(\'\[\', \'\]\'), $view->field(\'entry\').\'[<%=_.escape(position)%>][' . $v['handle'] . ']\'); ?>-<?php echo $uniqueID; ?> .redactor-editor {' . PHP_EOL;
                        $code .= BlockBuilderUtility::tab(8) . 'min-height: '.$height.'px !important;' . PHP_EOL;
                        $code .= BlockBuilderUtility::tab(7) . '}' . PHP_EOL;

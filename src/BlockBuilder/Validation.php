...
class Validation
{
    ...
    public function getForbiddenHandles() {
        ...
        $forbiddenWords = array('id', 'bID', 'position');
    ...
    public function validateBlockData($postData) {
        ...
        if (!$postData['blockHandle']) {
            ...
            if ($this->blockTypeFolderExists($postData['blockHandle'], true)) {
                ...
                if ($this->isBlockInstalled($postData['blockHandle'])) {
                    ...
                    $errors[] = t('Block with that handle is already installed. <a href="%s" target="_blank" rel="noopener">Uninstall it</a> first and then build block again or use another handle (%s).', URL::to('dashboard/blocks/types'.$urlEnding), t('Block settings'));
                    $fieldsWithError[] = 'blockHandle';
                    $tabsWithError[] = 'block-settings';
        ...
        $basicData   = $this->validateRepeatableEntries($postData['basic'], 'basic', t('Tab: Basic information'));
        $entriesData = $this->validateRepeatableEntries($postData['entries'], 'entries', t('Tab: Repeatable entries'));
    ...
    private function validateRepeatableEntries($postData, $handle, $label) {
        ...
        if ( is_array($postData) AND count($postData) ) {
            ...
            foreach ($postData as $counter => $entry) {
                ...
                if ( ! $entry['handle']) {
                    ...
                    if ( ! preg_match('/^[a-zA-Z]+$/', $entry['handle'])) {
                        $postData[$counter]['error']['handle'] = 1;
        ...
        if (in_array($handle.'|handle|invalid_characters', $fieldsWithError)) {
            $errors[] = t('There are some "Handle" fields which consist of characters other than a-zA-Z (%s).', $label);
        }
        ...
        $validation['tabsWithError'] = array();
        if (count($tabsWithError)) {
            $validation['tabsWithError'] = $tabsWithError;
        }

        $validation['fieldsWithError'] = array();

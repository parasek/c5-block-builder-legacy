        ...
        if (!$postData['blockHandle']) {
            ...
            if ($this->blockTypeFolderExists($postData['blockHandle'], true)) {
                ...
                if ($this->isBlockInstalled($postData['blockHandle'])) {
                    ...
                    $errors[] = t('Block with that handle is already installed. <a href="%s" target="_blank" rel="noopener">Uninstall it</a> first or use another handle (%s).', URL::to('dashboard/blocks/types'.$urlEnding), t('Block settings'));
                    $fieldsWithError[] = 'blockHandle';
                    $tabsWithError[] = 'block-settings';
